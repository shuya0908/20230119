...
\begin{remark*}
    ...
    Cameron-Martinの公式はCameron-Martin空間に属する関数 $h$ による平行移動の下でのWiener測度の「quasi-invariance（準不変性）」を与える．
    つまり（平行移動させる）写像 $w\mapsto w+h$ の下でのWiener測度 $W(dw)$ の像は $W(dw)$ に関する密度を持ち，この密度はmartingale $\int_0^t \dot{h}(s)dw(s)$ に関連するexponential martingaleの $t=\infty$ での値，つまり
    \begin{align}
...
\begin{proof}
    ...
    と
    \begin{align}
        \Phi(w)=\bm{1}_{\{\max_{[0,t]}w(s)\ge a\}}\text{ for }\Forall w\in C(\real_+,\real)
    \end{align}
    に対し，Cameron-Martinの公式を適用する：
    \begin{align}
        P(U_a\le t)
        = E[\Phi(B+h)]
        &= E\left[\Phi(B)\exp\left(\int_0^\infty \dot{h}(s)dB_s-\frac{1}{2}\int_0^\infty \dot{h}(s)^2 ds\right)\right] \\
    ...
    \begin{screen}
        ...
        \end{align}
        を示すためにoptional stopping theoremを適用し，さらにCor.2.22で与えられた $T_a$ の密度 $f(t)=\frac{a}{\sqrt{2\pi t^3}}e^{-\frac{a^2}{2t}}$ を用いる．
    \end{screen}
