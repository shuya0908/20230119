...
\begin{shadebox}
    \begin{prop}[Cameron-Martin formula]
    \label{thm:524}
...
\begin{remark*}
    積分 $\int_0^\infty\dot{h}(s)dw(s)$ は $W(dw)$ の下でのBMである $w(s)$ について確率積分であるが，関数 $\dot{h}(s)$ は決定論的なのでWiener積分とみなすこともできる．
    Cameron-Martinの公式は確率積分やGirsanovの定理を用いないGaussian calculationsで成立する．
    それでもこの公式ををGirsanovの定理の特別な例として導き出すことは有益である．

    Cameron-Martinの公式はCameron-Martin sp.に属する関数によるtranslationの下でWiener測度の「準不変性」を与える．
    この結果，
\end{remark*}
